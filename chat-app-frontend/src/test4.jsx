...
const ChatBox = ({ user }) => {
  ...
  const [showEmojiOnlyBox, setShowEmojiOnlyBox] = useState(true);
  ...
  {!showEmojiOnlyBox ? (
    <>
      <button
        ...
        onClick={() => setShowEmojiOnlyBox(true)}
      >
        😊
      </button>
      <input
      ...
      />
      <input className={styles.inputFiles} type="file" onChange={handleFileChange} />
      <button onClick={handleSend}>Send</button>
    </>
  ) : (
    <div className={styles.emojiOnlyBox}>
      <EmojiPicker
        onEmojiClick={(emojiData) => {
          setSelectedEmoji(emojiData.emoji);
          setNewMsg(emojiData.emoji);
          setShowEmojiOnlyBox(false);
        }}
      />
    </div>
